	...
	if($_SESSION['login']['ok'] == true)
	{
		...
		if(isset($_POST['action']))
		{
					...
					{  
						...
						if($item)
						{
							...
							if($bp > $cust->cusBonusPoints) 
							{
								...
								$msg = str_replace ('{GSSE_MSG_ERRORNEW}', $this->get_lngtext('LangTagTextBonusNotEnough'), $msg);
			...
			{	
				...
				$tmplFile = "errorbox.html";
				$msg = $this->gs_file_get_contents('template/' . $tmplFile);
				$msg = str_replace ('{GSSE_MSG_ERRORNEW}', $this->get_lngtext('LangTagTextBonusEmpty'), $msg);
				$msg = str_replace('{GSSE_MSG_ERRORNEWCLASS}', 'error', $msg);
				$this->content = str_replace ('{GSSE_MSG_}', $msg, $this->content);
			}
		}
		...
		if(file_exists("dynsb/class/class.shoplog.php"))
		{
			...
			$sl = new shoplog();
			$cust = $sl->getCustData($_SESSION['login']['cusIdNo']);
			...
			if($num==0)
			{
				...
				$this->content = str_replace ('{GSSE_FUNC_BONUSESNEW}', $this->get_lngtext('LangTagNoBonusArticles'), $this->content);
			}
			else
			{
				...
				$this->content = str_replace ('{GSSE_FUNC_BONUSESNEW}', $msg, $this->content);
			}
			// End Bonus Artikel
		}
		else 
