...
function customer_textmail($doSendMail) {
	...
	if($doSendMail) {
		...
		$me->sendmail($_POST['email'], $_POST['answer_subject'], '', '', false);
	}
	return;
}
...
function shopowner_htmlmail() {
	...
	die("</pre>");*/
		...
		//$succ = $me->sendmail($_POST['email'], $_POST['answer_subject'], '', '', true);
		$me->sendmail($_POST['recipient'], $_POST['subject'], $attach, 'gsshopbuilder.txt', true);
...
function customer_pwmail() {
	...
	if($lSendMail) {
		...
		$me->sendmail($_POST['email'], $_POST['userdata'] . ' ' . $_POST['shopname'],'', '', false);
	}
